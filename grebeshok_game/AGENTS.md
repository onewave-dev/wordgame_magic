...
Во время активной игры (как в обычном раунде, так и в админской тестовой
версии) ответ рассылается во все чаты игроков, указанные в `game.player_chats`.
В сообщении указывается, кто сделал запрос, какое слово проверялось, есть ли оно
в словаре и добавляется определение из LLM/Викисловаря. Запросивший игрок тоже
получает эту рассылку; если доставка в его чат не удалась, бот повторно отправит
ответ напрямую в приватный чат запроса. Если активной игры нет, сообщение
возвращается только инициатору и другим игрокам не показывается.
