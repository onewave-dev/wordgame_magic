...
- Игра реализуется по принципу **Hub-and-Spoke**: каждый игрок играет в своём **личном чате (DM)** с ботом. Бот выступает как “хаб”: собирает слова от всех игроков, ведёт счёт, а затем рассылает общие результаты и игровые события **отдельным сообщением в личку каждому игроку**.  
  ❗ Все упоминания «чат игры» в тексте ниже означают **одновременную рассылку сообщений ботом в лички всем активным игрокам**, а не отдельный групповой чат.  
...
- Дополнительные требования архитектуры:  
  ...
  - build in auto registering webhook at start-up (conditionally, if re-registration is necessary)  
...
3. Бот публикует каждому игроку в личку:  
   - Кнопки «Пригласить из контактов», «Создать ссылку» (inline) и «Присоединиться»  
   - deep-link для пересылки друзьям  

4. Игроки жмут «Присоединиться».  
   Если имени нет → бот запрашивает (через ForceReply). Ввод имени всех игроков обязателен.  
